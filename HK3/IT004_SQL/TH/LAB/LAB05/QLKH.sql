...
SELECT HD.SOHD
FROM HOADON HD INNER JOIN CTHD CT ON HD.SOHD = CT.SOHD
WHERE (CT.MASP = 'BB01' OR CT.MASP = 'BB02') AND CT.SL BETWEEN 10 AND 20;
...
FROM SANPHAM SP INNER JOIN CTHD CT ON SP.MASP = CT.MASP
				INNER JOIN HOADON HD ON CT.SOHD = HD.SOHD
WHERE SP.NUOCSX = 'Trung Quoc' OR HD.NGHD = '2007-1-1';
...
FROM SANPHAM SP INNER JOIN CTHD CT ON SP.MASP = CT.MASP
				INNER JOIN HOADON HD ON CT.SOHD = HD.SOHD
WHERE CT.SL = NULL AND YEAR(HD.NGHD) = '2006';

--17
SELECT SP.MASP, SP.TENSP
FROM SANPHAM SP INNER JOIN CTHD CT ON SP.MASP = CT.MASP
				INNER JOIN HOADON HD ON CT.SOHD = HD.SOHD
WHERE SP.NUOCSX = 'Trung Quoc' AND CT.SL = NULL AND YEAR(HD.NGHD) = '2006';

--18
SELECT HD.SOHD
FROM HOADON HD INNER JOIN CTHD CT ON HD.SOHD = CT.SOHD
WHERE CT.MASP = ALL (SELECT MASP
					FROM SANPHAM 
...
FROM HOADON HD INNER JOIN CTHD CT ON HD.SOHD = CT.SOHD
WHERE YEAR(HD.NGHD) = '2006' AND  CT.MASP >= ALL (SELECT MASP
													FROM SANPHAM 
													WHERE NUOCSX = 'Singapore');

--20
SELECT COUNT(HD.SOHD) AS KHKHONGDK
FROM HOADON HD INNER JOIN KHACHHANG KH ON HD.MAKH = KH.MAKH
WHERE KH.NGDK = NULL;

--21
SELECT COUNT(DISTINCT CT.MASP) AS SLSP
...
--32
SELECT COUNT(MASP) AS TONG_SP_TQ
FROM SANPHAM
WHERE NUOCSX = 'Trung Quoc';
...
--34
SELECT NUOCSX,
MAX(GIA) AS GIA_BAN_CAO_NHAT,
MIN(GIA) AS GIA_BAN_THAP_NHAT,
AVG(GIA) AS GIA_BAN_TRUNG_BINH
FROM SANPHAM
GROUP BY NUOCSX;

--35
SELECT NGHD, SUM(TRIGIA) AS DOANH_THU_MOI_NGAY
FROM HOADON
GROUP BY NGHD;

--36
SELECT CT.MASP, SUM(CT.SL) AS TONG_SL_SP
FROM CTHD CT INNER JOIN HOADON HD ON CT.SOHD = HD.SOHD
...
SUM(TRIGIA) AS DOANH_THU_MOI_THANG
FROM HOADON
WHERE YEAR(NGHD) = '2006'
GROUP BY MONTH(NGHD);
...
--40
SELECT TOP 1 k.MAKH, k.HOTEN, COUNT(h.MAKH) AS SoLanMua
...
--42
SELECT TOP 1 SP.MASP, SP.TENSP, SUM(CT.SL) AS SOLUONG
...
FROM SANPHAM SP
WHERE SP.GIA = (SELECT MAX(GIA)
			FROM SANPHAM 
			WHERE NUOCSX = SP.NUOCSX);

--44
SELECT NUOCSX, COUNT(MASP) AS SO_SP, COUNT(DISTINCT GIA) AS SO_GIA_BAN
FROM SANPHAM
GROUP BY NUOCSX
...
SoLanMua AS (
    SELECT k.MAKH, COUNT(h.MAKH) AS SoLanMua
    FROM KhachHang k
    JOIN HoaDon h ON k.MAKH = h.MAKH
    WHERE k.MAKH IN (SELECT MAKH FROM Top10KhachHang)
    GROUP BY k.MAKH
)
SELECT TOP 1 k.MAKH, k.HOTEN, slm.SoLanMua
FROM KhachHang k
JOIN SoLanMua slm ON k.MAKH = slm.MAKH
ORDER BY slm.SoLanMua DESC;

--I
--11
CREATE TRIGGER NGHD_DK_INSERTED
ON HOADON
FOR INSERT, UPDATE
AS
	IF EXISTS (
		...
		WHERE NGDK <> NULL AND NGHD < NGDK
		)
	BEGIN 
		PRINT 'NGAY MUA HANG NHO HON NGAY DANG KI THANH VIEN'
		ROLLBACK TRAN
	END;

--12
CREATE TRIGGER NGHD_VL_INSERTED
ON HOADON
FOR INSERT, UPDATE
AS
	IF EXISTS (
	...
	BEGIN 
		PRINT 'NGAY BAN HANG NHO HON NGAY VAO LAM'
		ROLLBACK TRAN
	END;

--13
CREATE TRIGGER HOADON_CT
ON HOADON 
FOR DELETE 
AS 
	IF NOT EXISTS (
		SELECT 1
		FROM DELETED D JOIN CTHD CT ON D.SOHD = CT.SOHD
		)
	BEGIN 
		PRINT 'HOA DON PHAI CO IT NHAT 1 CHI TIET HOA DON'
		ROLLBACK TRAN
	END;

--14
CREATE TRIGGER TG_HOADON_TRIGIA
ON HOADON 
FOR INSERT, UPDATE
AS 
	IF EXISTS (
		SELECT 1
		FROM INSERTED I JOIN CTHD CT ON I.SOHD = CT.SOHD
						JOIN SANPHAM SP ON CT.MASP = SP.MASP
		GROUP BY I.SOHD, I.TRIGIA
		HAVING TRIGIA <> SUM(GIA * SL)
		)
	BEGIN 
		PRINT 'TRI GIA SAI'
		ROLLBACK TRAN
