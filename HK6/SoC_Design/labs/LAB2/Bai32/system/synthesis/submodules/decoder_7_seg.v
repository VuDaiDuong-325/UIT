module decoder_7_seg (
    ...
    input              iWrite_n,
    input      [31:0]  iWrite_data,    // Dữ liệu từ CPU (4-bit thấp)
    output reg [6:0]   oHEX_physical   // Nối với chân HEX vật lý (Conduit)
);
    // Logic giải mã LED 7 đoạn (Anode chung)
    always @(posedge iClk or negedge iReset_n) begin
        if (~iReset_n)
            oHEX_physical <= 7'b1111111; // Tắt hết
